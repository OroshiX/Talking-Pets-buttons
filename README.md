...
- `talking_pet_buttons/talking_pet_buttons.ino`: sketch Arduino pour 24 boutons.
- `docs/cablage.md`: schema de cablage et logique electrique.
- `docs/liste-achats.md`: composants recommandes avec liens France/UE.
- `docs/mise-en-route.md`: installation Arduino IDE, cle USB et premiers tests.
...
- La sortie audio jack 3,5 mm du GIGA va vers une enceinte amplifiee AUX.

## Demarrage rapide

1. Installer le package de carte `Arduino Mbed OS GIGA Boards`.
2. Installer les bibliotheques `Arduino_AdvancedAnalog` et
   `Arduino_USBHostMbed5`.
3. Formater une cle USB en FAT32 avec schema MBR.
4. Copier des fichiers WAV mono 16-bit PCM a la racine de la cle, avec les noms
   listes dans `audio/README.md`.
5. Ouvrir `talking_pet_buttons/talking_pet_buttons.ino` dans Arduino IDE.
6. Televerser sur l'Arduino GIGA R1 WiFi.
7. Brancher un bouton entre `D2` et `GND`, puis ouvrir le moniteur serie a
