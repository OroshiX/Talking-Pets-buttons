...
bool skipBytes(FILE *file, uint32_t byteCount) {
...
void printButtonMap() {
  ...
  for (uint8_t i = 0; i < kButtonCount; i++) {
    ...
    Serial.print(" -> ");
    Serial.print(kButtons[i].word);
    Serial.print(" -> ");
    Serial.println(kButtons[i].filePath);
  }
}
...
void streamCurrentSound() {
  ...
  const size_t samplesLeft = static_cast<size_t>(bytesRemaining / currentSampleSize);
